...
import {
  getClearingSessionPayload,
  getParticipantsFromDb,
  listClearingSessions,
} from "../api";
import {
  ClearingAuditResult,
  ClearingSessionSummary,
...
function renderSessionDetails(
  ...
  return (
    ...
    >
      <div
        style={{ background: "#f8fafc", borderRadius: "6px", padding: "8px" }}
      >
        <div>
          <b>Хеш результата:</b>{" "}
          <span style={{ fontFamily: "monospace" }}>{audit.hash}</span>
        </div>
        <div>
          <b>Корень Merkle:</b>{" "}
        ...
        <div>
          <b>Решатель:</b> {audit.solver_version || "н/д"} | <b>Сборка:</b>{" "}
          {audit.build_sha || "н/д"}
        </div>
        {audit.fallback_reason ? (
          <div>
            <b>Причина fallback:</b> {audit.fallback_reason}
          </div>
        ) : null}
        {audit.flow_objective ? (
          <div>
            <b>Цель flow:</b> {audit.flow_objective}
          </div>
        ) : null}
        {audit.flow_total_cost != null ? (
          <div>
            <b>Суммарная стоимость flow:</b> {audit.flow_total_cost}
          </div>
        ) : null}
        {audit.flow_unmet_demand != null ? (
          <div>
            <b>Непокрытый спрос flow:</b> {audit.flow_unmet_demand}
          </div>
        ) : null}
        <div>
          <b>Создано:</b> {fmtTs(audit.timestamp)}
        </div>
      </div>
      ...
      >
        ...
        <p style={{ margin: "0 0 8px 0", color: "#64748b", fontSize: "12px" }}>
          Нетто: входящие минус исходящие только по внешним аллокациям (реальные
          платежи между контрагентами). Положительное — получатель,
          отрицательное — платящий.
        </p>
        {counterpartiesNet.length === 0 ? (
          <span style={{ color: "#64748b" }}>
            Нет участников во входном снимке обязательств
          </span>
        ) : (
          <table
            style={{
                ...
                <tr key={pubkey} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  ...
                  <td
                    style={{
                      padding: "6px 4px",
                      color:
                        net > 0 ? "#166534" : net < 0 ? "#b91c1c" : "#64748b",
                    }}
                  >
                    {net === 0
                      ? "0 SOL"
      ...
      {!!audit.input_obligations?.length && (
        <details>
          <summary style={{ cursor: "pointer" }}>
            Входные обязательства ({audit.input_obligations.length})
          </summary>
          <table
            style={{
              width: "100%",
      ...
      <details>
        <summary style={{ cursor: "pointer" }}>
          Внешние аллокации ({audit.data.length})
        </summary>
        <ul>
          {audit.data.map((x) => (
      ...
      <details>
        <summary style={{ cursor: "pointer" }}>
          Внутренние неттинги ({audit.internal_data.length})
        </summary>
        <ul>
      ...
      <details>
        <summary style={{ cursor: "pointer" }}>
          Хронология аудита ({audit.audit_log.length})
        </summary>
        <ul>
      ...
      <details>
        <summary style={{ cursor: "pointer" }}>
          Визуальный граф и дерево Merkle
        </summary>
...
export default function AdminSessionsPage() {
  ...
  const [expandedSessions, setExpandedSessions] = useState<
    Record<number, ClearingAuditResult>
  >({});
  const [openedSessionIds, setOpenedSessionIds] = useState<Set<number>>(
    new Set(),
  );
  const [loading, setLoading] = useState(false);
  const [walletFilter, setWalletFilter] = useState("");
  const [statusFilter, setStatusFilter] = useState<
    "all" | "has_external" | "has_internal" | "empty"
  >("all");
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const [page, setPage] = useState(1);
  const [participantsDirectory, setParticipantsDirectory] = useState<
    ParticipantDirectoryEntry[]
  >([]);
  const PAGE_SIZE = 10;

  const participantNameByAuthority = useMemo(() => {
    return participantsDirectory.reduce<Record<string, string>>(
      (acc, participant) => {
  ...
  const [walletMatchedIds, setWalletMatchedIds] = useState<Set<number> | null>(
    null,
  );
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const s of sessions) {
        try {
          ...
          const haystack = [
            ...(payload.input_obligations?.flatMap((o) => [
              o.obligation,
              o.from,
              o.to,
            ]) || []),
  ...
  const filtered = useMemo(() => {
    const fromTs = dateFrom
      ...
      : null;
    const toTs = dateTo
      ? new Date(`${dateTo}T23:59:59.999`).getTime() / 1000
      : null;
    return sessions.filter((s) => {
      if (statusFilter === "has_external" && s.external_count <= 0)
        return false;
      if (statusFilter === "has_internal" && s.internal_count <= 0)
        return false;
      if (
        statusFilter === "empty" &&
        (s.external_count > 0 || s.internal_count > 0)
      )
        return false;
      ...
      if (
        walletFilter.trim() &&
        walletMatchedIds &&
        !walletMatchedIds.has(s.session_id)
      )
        return false;
      return true;
    });
  ...
  const exportCsv = () => {
    ...
    const csv = [headers, ...rows]
      .map((row) =>
        row
          .map((x) => csvEscape(x as string | number | null | undefined))
          .join(","),
      )
      .join("\n");
        ...
        <div style={{ display: "grid", gap: "8px" }}>
          {pageItems.map((s) => (
            ...
            >
              ...
              >
                {openedSessionIds.has(s.session_id) ? "▼" : "▶"} Сессия #
                {s.session_id} | результат: {s.result_id} | внешние:{" "}
                {s.external_count}, внутренние: {s.internal_count}
              </div>
              <div
                style={{ fontSize: "12px", color: "#64748b", marginTop: "4px" }}
              >
                создано: {fmtTs(s.created_at)} | merkle:{" "}
                {shortKey(s.merkle_root || "н/д")}
              </div>
