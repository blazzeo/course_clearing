...
export default function MultiPartyClearing() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (participants.length < 2) {
			toast.error('Необходимо минимум 2 участника')
			return
		}

		if (participants.some(p => !p.trim())) {
			toast.error('Заполните все адреса участников')
			return
		}
		...
		try {
			...
			const response = await axios.post(`${API_URL}/api/clearing/run`, {
				participants: participants.filter(p => p.trim()),
				amounts: amountsLamports,
			})

			if (response.data.success) {
				toast.success('Многосторонний клиринг создан успешно!')
				navigate('/positions')
			}
		} catch (error: any) {
			console.error('Error creating multi-party clearing:', error)
			toast.error(error.response?.data?.error || 'Ошибка при создании многостороннего клиринга')
		} finally {
			setLoading(false)
		}
	}
