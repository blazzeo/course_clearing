...
export default function AdminObligationsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((o) => {
      if (statusFilter !== ObligationStatus.All && o.status !== statusFilter)
        return false;
      if (wallet) {
  ...
  return (
    <div className="card">
      ...
      >
        <h2
          id="admin-oblig-filters-heading"
          style={{
            fontSize: "15px",
            fontWeight: 700,
            margin: "0 0 8px 0",
        ...
        >
          Данные из базы индексатора. Условия суммируются: в списке остаются
          только те позиции, которые подходят <strong>по всем</strong> выбранным
          критериям сразу.
        </p>
        ...
        >
          <div>
            ...
            />
            <p style={hint}>
              Совпадение с адресом дебитора или кредитора. Пусто — все
              участники.
            </p>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value={ObligationStatus.Created}>
                Ожидает подтверждения
              </option>
              <option value={ObligationStatus.Confirmed}>Подтверждено</option>
              <option value={ObligationStatus.PartiallyNetted}>
                Частично погашено
              </option>
            ...
            <p style={hint}>
              Как сохранено индексатором после событий по цепочке.
            </p>
          </div>
          ...
          <div>
            ...
            <p style={hint}>
              Границы включительно, в локальном времени браузера. Пустое поле —
        ...
        {!loading && items.length > 0 && (
          ...
          >
            Совпало с условиями: <strong>{filtered.length}</strong> из{" "}
            {items.length}
      ...
      {loading ? (
        ...
        <p style={{ color: "#666", textAlign: "center", padding: "24px" }}>
          Ничего не подошло под фильтры или список пуст
        </p>
      ) : (
        <>
            ...
            <tbody>
              {pageItems.map((o) => (
                <tr key={o.pda.toBase58()}>
                  <td>
                    <Link to={`/participant/${o.from.toBase58()}`}>
                      {o.from.toBase58().slice(0, 8)}...
                    </Link>
                  </td>
                  <td>
                    <Link to={`/participant/${o.to.toBase58()}`}>
                      {o.to.toBase58().slice(0, 8)}...
                    </Link>
                  </td>
                  <td>
                    {(o.amount / 1e9).toFixed(4)}
                    {o.originalAmount != null
                      ? ` / ${(o.originalAmount / 1e9).toFixed(4)}`
                      : ""}{" "}
                    SOL
                  </td>
                  <td>
