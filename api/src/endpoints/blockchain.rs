...
/// Создать инструкцию для оплаты комиссии
pub async fn create_collect_fee_instruction_endpoint(
    blockchain_client: web::Data<Arc<crate::blockchain::BlockchainClient>>,
    req: web::Json<crate::models::CollectFeeRequest>,
    query: web::Query<std::collections::HashMap<String, String>>,
) -> impl Responder {
    let address = match query.get("address") {
        Some(addr) => match Pubkey::from_str(addr) {
    ...
    match blockchain_client
        .create_collect_fee_instruction(&address, req.amount, req.reason.clone())
        .await
    {
        Ok(instruction) => HttpResponse::Ok().json(ApiResponse::success(serde_json::json!({
            "instruction": {
...
pub async fn create_withdraw_system_fees_instruction_endpoint(
    blockchain_client: web::Data<Arc<crate::blockchain::BlockchainClient>>,
    req: web::Json<crate::models::WithdrawFeesRequest>,
    query: web::Query<std::collections::HashMap<String, String>>,
) -> impl Responder {
    let _address = match query.get("address") {
        Some(addr) => match Pubkey::from_str(addr) {
            Ok(pubkey) => pubkey,
            Err(_) => {
                return HttpResponse::BadRequest().json(ApiResponse::<String>::error(
    ...
    match blockchain_client
        .create_withdraw_system_fees_instruction(&recipient, req.amount)
        .await
    {
        Ok(instruction) => HttpResponse::Ok().json(ApiResponse::success(serde_json::json!({
            "instruction": {
...
pub async fn get_escrow_address(
    ...
    let (escrow_pda, _) = blockchain_client.get_escrow_pda().await;

    HttpResponse::Ok().json(ApiResponse::<String>::success(escrow_pda.to_string()))
}
