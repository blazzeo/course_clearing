...
use clearing_service::{
    instruction::{
        ...
        RequestWithdrawal, WithdrawSystemFees,
    },
    Participant,
};
...
impl BlockchainClient {
    ...
    pub async fn create_collect_fee_instruction(
        &self,
        participant_authority: &Pubkey,
        amount: u64,
        reason: String,
    ) -> Result<Instruction> {
        let (participant_pda, _) = self.get_participant_pda(participant_authority).await;
        ...
        let accounts = vec![
            ...
            AccountMeta::new(*participant_authority, true),
        ];
    ...
    /// Создать инструкцию для вывода системных комиссий
    pub async fn create_withdraw_system_fees_instruction(
        &self,
        recipient: &Pubkey,
        amount: u64,
    ) -> Result<Instruction> {
        let (escrow_pda, _) = self.get_escrow_pda().await;
        let escrow_authority = self.get_escrow_authority().await?;

        let data = WithdrawSystemFees { amount }.data();
    ...
    /// Получить authority escrow аккаунта (админа)
    pub async fn get_escrow_authority(&self) -> Result<Pubkey> {
        let (escrow_pda, _) = self.get_escrow_pda().await;
        ...
        let data = &account.data[8..]; // Пропускаем discriminator
        let authority_bytes = &data[0..32]; // authority - первые 32 байта
        let authority =
            Pubkey::try_from(authority_bytes).map_err(|_| anyhow!("Invalid authority data"))?;
